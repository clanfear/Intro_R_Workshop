...
This will download and install `readr` from the Comprehensive R Archive Network (CRAN).
...
Also note here that I loaded data directly from an online source.
]
]
...
library(gapminder)
data(gapminder)
head(gapminder) # head() shows first 6 elements of an object #<<
```
...
.footnote[
Note we always need to use `gapminder$` to subset gapminder (rather than just `country == "Oman"` alone. This is because you can subset one object using a completely different object. This is actually very powerful!
]
...
.footnote[
Note that you can split arguments to a function into multiple lines for clarity, so long as lines end with an operator (like `,` or `+`).
]
