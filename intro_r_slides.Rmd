...
date: "Oct 11, 2018<br>Updated: `r gsub(' 0', ' ', format(Sys.Date(), format='%b %d, %Y'))`"
output:
...
Most data sets you will work with will be read into R and stored as a **dataframe**, so the remainder of this workshop will mainly focus on manipulating these objects.
...
R has a variety of built-in functions for importing data stored in text files, like `read.table()` and `read.csv()`. I recommend using the versions in the `readr` package instead: `read_csv()`, `read_tsv()`, and `read_delim()`:<sup>1</sup>
...
.footnote[[1] Use "write" versions (e.g. `write_csv()`) to create these files from R objects.
]
...
The real power of R is found in add-on packages. Packages contain functions (and sometimes data) created by the community.

To install the `readr` package, use the following:

```{r, eval=FALSE}
install.packages("readr")
```

This will download and install `readr` from the Central R Archive Network (CRAN).

You only need to install a package **once** for any given version of R. You want to reinstall packages after upgrading R.
...
Once a package is loaded, you can call on functions inside it.
...
.small[
```{r}
spd_data <- read_csv("https://clanfear.github.io/CSSS508/Seattle_Police_Department_911_Incident_Response.csv")
```
]

.pull-right30[
.footnote[When loading a file, `readr` guesses at the format of each column by looking at the first 1000 rows. This output indicates the "guesses" made by `readr`. Always check to be sure they make sense!]
]
...
* `foreign`
    ...
    + Less complex but loses some metadata
...
These data are a panel of 142 countries observed every 5 years from 1952 to 2007.
...
.small[
```{r}
gapminder[1:4, 3:4] # First four rows, third and fourth column #<<
```
]

.pull-right[
.footnote[
The **colon operator** (`:`) generates a vector using the sequence of integers from its first argument to its second. `1:4` is equivalent to `c(1,2,3,4)`.
]
]
---
...
Columns in dataframes can also be accessed using their names. This will return the column as a vector:
...
You can mix subsetting formats! In this case I provided only a single value (no column index) because **vectors** have *only one dimension*. 
...
What does `gapminder$country=="Oman"` actually do? 

--

```{r}
head(gapminder$country=="Oman", 50)
```

--

It returns a vector of `TRUE` or `FALSE` values. When used with the subset operator (`[]`), elements for which a `TRUE` is given are returned while those corresponding to `FALSE` are dropped.

Note I displayed only the first 50 here... and there are no observations from Oman in the first 50 rows of the data!
...
We used `==` for testing "equals": `gapminder$country == "Oman"`.
...
.footnote[
Note we always need to use `gapminder$` to subset gapminder. This is because you can subset one object using a completely different object. This is actually very powerful!
]
...
.footnote[[1] Remember the `c()` function is how we make **vectors** in R.]
...
yugoslavia$short_country <-  ifelse(yugoslavia$country == "Bosnia and Herzegovina",
                                    "B and H", as.character(yugoslavia$country))
yugoslavia[yugoslavia$year==1952, c(1,9)]
```
...
.pull-right[
.footnote[Note that `lifeExp ~ gdpPercap` is a **formula** of the type `y ~ x`. The first element (`dist`) gets plotted on the y-axis and the second (`speed`) goes on the x-axis. Regression formulae follow this convention as well!
...
We can run an ordinary least squares linear regression using the `lm()` function:
...
The `summary()` function provides a Stata-like regression output:
...
We can also do this with `summary()`:
...
.footnote[
Note that `summary()` calculates and provides standard errors and test statistics.
]
---
...
* `glm()` has syntax similar to `lm()` but adds a `family =` argument to specify model families and link functions like logistic regression (`glm(x~y, family = binomial(link="logit"))`)
* The `lme4` package adds hierarchical (multilevel) linear models.
* `lavaan` adds structural equation models.
...
* [CSSS508: Introduction to R for Social Sciences](https://clanfear.github.io/CSSS508/)

* [Installing R and RStudio](https://clanfear.github.io/CSSS508/docs/installation.html)
